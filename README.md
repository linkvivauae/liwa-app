...
| Web output | Single-page export (`app.json` → `web.output: "single"`) |
...
- The workflow copies `index.html` to `404.html`. This is a single-page build, so routes like `/races` have no file of their own; Pages serves `404.html` for unknown paths, which hands control to the client router. Any host that rewrites unknown paths to `index.html` works without this step.
...
- **The web target is a single-page build, deliberately.** It was originally `web.output: "static"` (prerendered). Static rendering triggered a React hydration mismatch inside `@expo/vector-icons` — the icon font is not resolved at prerender time, so the server HTML disagreed with the client. React discarded and regenerated the tree, which interrupted the Reanimated entry animations and left booking and news cards stranded at partial opacity. Switching to `"single"` removes the prerender step, so there is no mismatch and the animations complete. The trade-off is no server-side rendering, which costs nothing here — this is a member app, not indexable marketing content. The deploy workflow copies `index.html` to `404.html` so deep links such as `/races` still resolve on GitHub Pages.
